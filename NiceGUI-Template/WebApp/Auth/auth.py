...
app.add_middleware(
    SessionMiddleware, secret_key="some_random_string"
...
def main_page(request: Request) -> None:
    ...
    with ui.column().classes("absolute-center items-center"):
        ...
        ui.button("", on_click=lambda: ui.open("/logout")).props(
            "outline round icon=logout"
        )
...
def login(request: Request) -> None:
    def try_login() -> (
        ...
        response = client.query(
            f"SELECT * FROM user where username='{username.value}' && password='{password.value}'"
        )
    ...
    request.session["id"] = str(
        uuid.uuid4()
    )  # NOTE this stores a new session ID in the cookie of the client
    with ui.card().classes("absolute-center w-96 items-center items-stretch"):
        ...
        password = (
            ui.input("Password").props("type=password").on("keydown.enter", try_login)
        )
